...
| `meshfree_kl_shell` | Meshfree RKPM Kirchhoff–Love shell (`RKShellT`, Wang & Bazilevs 2025): rotation-free, nodally integrated with natural stabilization, plane-stress J2 plasticity, co-rotation, self-contact; docs in `docs/meshfree_kl_shell/` (#59) |
...
|-------|------|------------|------|-------|
| level.0 | **173** | 14 | — | Core physics suite (2026-09-22 run). The 14 failures are the #37 set: 9 need `BRIDGING_ELEMENT`, plus CSE.2, adhesion.{2,3}, inputoutput/square (counted twice by the runner) |
...
- `doc/user_guide.pdf` — Tahoe User Guide (sources in `doc/src`, build with `pdflatex tahoe.user && bibtex tahoe.user` and two more `pdflatex` passes). Updated in 2026 with the meshfree Kirchhoff–Love shell, essential boundary conditions for meshfree fields, the regression-test system, and complete controller tables.
- `docs/meshfree_kl_shell/technical_report.pdf` — technical report on the meshfree shell: formulation, boundary conditions, implementation, verification against Wang & Bazilevs (2025), tests and CI.
- `benchmark_XML/ReadMe` and the per-level `README.md` files — running and comparing the regression tests.
