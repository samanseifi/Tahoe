...
/** interface to SuperLU_MT 1.0 multi-threaded linear solver */
class SuperLU_MTMatrixT: public GlobalMatrixT
{
public:

	/** constructor */
	SuperLU_MTMatrixT(ostream& out, int check_code, int num_threads, const CommunicatorT& comm);

	/** destructor */
	~SuperLU_MTMatrixT(void);
	...
	/** assignment operator */
	SuperLU_MTMatrixT& operator=(const SuperLU_MTMatrixT& rhs);
...
protected:
	...
	friend ostream& operator<<(ostream& out, const SuperLU_MTMatrixT& matrix) {
...
private:
	...
	/** no copy constructor */
	SuperLU_MTMatrixT(const SuperLU_MTMatrixT&);

protected:

	/** number of threads to use in calls to SuperLU_MT library */
	int fNumThreads;
...
inline double& SuperLU_MTMatrixT::operator()(int row, int col)
{
	const char caller[] = "SuperLU_MTMatrixT::operator()";
...
/* element accessor - READ ONLY */
inline double SuperLU_MTMatrixT::Element(int row, int col) const
{
	const char caller[] = "SuperLU_MTMatrixT::Element()";
...
#endif /* _SUPER_LU_MT_MATRIX_T_H_ */
