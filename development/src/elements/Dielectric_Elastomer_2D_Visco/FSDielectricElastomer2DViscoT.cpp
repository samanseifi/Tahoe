...
/* Destructor */
  FSDielectricElastomer2DViscoT::~FSDielectricElastomer2DViscoT()
  {
	  if (0 != fFSDEMatSupport2DVisco) delete fFSDEMatSupport2DVisco;
  }

  //
  // specify parameters needed by the interface
  //
  void FSDielectricElastomer2DViscoT::DefineParameters(ParameterListT& list) const
  {
//  	cout << "FSDielectricElastomer2DViscoT::DefineParameters" << endl;
    // inherited
  ...
  //
  void FSDielectricElastomer2DViscoT::TakeParameterList(const ParameterListT& list)
  {
    //
    // inherited
    //
    ...
    if (!fElectricScalarPotentialField) {
      ExceptionT::GeneralFail("FSDielectricElastomer2DViscoT::TakeParameterList",
...
GlobalT::SystemTypeT FSDielectricElastomer2DViscoT::TangentType(void) const
{
  ...
  //
  MaterialSupportT*
  FSDielectricElastomer2DViscoT::NewMaterialSupport(MaterialSupportT* p) const
  {
    //
    // allocate
    //
    if (!p) p = new FSDEMatSupport2DViscoT(1, NumIP());
    ...
    //
    FSDEMatSupport2DViscoT* ps = dynamic_cast<FSDEMatSupport2DViscoT*> (p);
  ...
  //
  MaterialListT*
  FSDielectricElastomer2DViscoT::NewMaterialList(const StringT& name, int size)
  {
    ...
    if (size > 0) {
      ...
      // material support
      //
      if (0 == fFSDEMatSupport2DVisco) {
        fFSDEMatSupport2DVisco = dynamic_cast<FSDEMatSupport2DViscoT*> (NewMaterialSupport());

        if (0 == fFSDEMatSupport2DVisco) {
          ExceptionT::GeneralFail("FSDielectricElastomer2DViscoT::NewMaterialList");
        }
      }

      mlp = new FSSolidMatList2DT(size, *fFSDEMatSupport2DVisco);
  ...
  void FSDielectricElastomer2DViscoT::SetGlobalShape()
  {
    //
    // inherited
    //
  ...
  //
  void FSDielectricElastomer2DViscoT::CurrElementInfo(ostream& out) const
  {
    //
    // inherited
    //
...
// 
  bool FSDielectricElastomer2DViscoT::NextElement()
  {
    ...
    if (isThereNext == true) {
      ...
      fCurrMaterial = dynamic_cast<FSDEMat2DViscoT*> (pMaterial);
  ...
  //
  void FSDielectricElastomer2DViscoT::SetLocalArrays()
  {
    //
    ...
    if (0 == esp) {
      ...
      std::cout << std::endl;
      std::cout << "FSDielectricElastomer2DViscoT::SetLocalArrays: ";
  ...
  //
  //
  void FSDielectricElastomer2DViscoT::Equations(AutoArrayT<const iArray2DT*>& eq_1,
...
void FSDielectricElastomer2DViscoT::AddNodalForce(const FieldT& field, int node, dArrayT& force)
{
	/* not my field */
//	if (&field != &(Field())) return;
  ...
  //
  void FSDielectricElastomer2DViscoT::Set_B_C(const dArray2DT& DNaX, dMatrixT& B_C)
  {
  ...
  // Geometric stiffness
  //
  void FSDielectricElastomer2DViscoT::AccumulateGeometricStiffness(dMatrixT& Kg,
...
/* calculate the LHS of residual, or element stiffness matrix */
  void FSDielectricElastomer2DViscoT::FormStiffness(double constK)
  {
...
void FSDielectricElastomer2DViscoT::FormMass(MassTypeT mass_type, double constM, bool axisymmetric, const double* ip_weight)
{
...
void FSDielectricElastomer2DViscoT::FormMa(MassTypeT mass_type, double constM, bool axisymmetric, 
	const LocalArrayT* nodal_values, const dArray2DT* ip_values, const double* ip_weight)
{
	const char caller[] = "FSDielectricElastomer2DViscoT::FormMa";
...
void FSDielectricElastomer2DViscoT::MassMatrix()
{
...
/* Compute RHS, or residual of element equations */
  void FSDielectricElastomer2DViscoT::FormKd(double constK)
  {
    /* element preliminaries */
    const int nsd = NumSD();
    const int nen = NumElementNodes();
