/* $Id: C0_LineT.cpp,v 1.1 2004/09/02 18:25:08 rdorgan Exp $ */ 
#include "C0_LineT.h"
#include "ifstreamT.h"
#include "StringT.h"
...
C0_LineT::C0_LineT(GeometryT::CodeT geometry_code, int numip, int numnd_u,  int numdof_u, const LocalArrayT& coords):
	ShapeTools     (),
	...
	fNumNodes_X    (fCoords.NumberOfNodes()), // only 2 or 3
	fNumNodes_U    (numnd_u),                 // only 2 or 3
	fNumDOF_X      (fNumSD),                  // only 1
	...
	fMatx1         (fNumSD)
{
	const char caller[] = "C0_LineT::Constructor";
	...
	if (fNumNodes_U != 2 && fNumNodes_U != 3)
		ExceptionT::BadInputValue(caller, "numnode_u != 2 && numnode_u != 3 (nsd=1)", fNumNodes_U);
	if (fNumIP != 1 &&
		fNumIP != 2 && 
		fNumIP != 3 && 
		fNumIP != 4)
	...
	if (fNumDOF_U != 1)
		ExceptionT::BadInputValue(caller, "numdof_u != 1 (nsd=1)");
...
void C0_LineT::SetDerivatives()
{
	const char caller[] = "C0_LineT::ComputeGDNaU";
...
void C0_LineT::SetLocalShape()
{
	const char caller[] = "C0_LineT::SetLocalShape";
	...
	switch (fNumNodes_U)
		...
		case 2: {         
			...
			{	
				...
				/* Na */
		    	na_field[0] = 0.5*(1.0 - xa[i]);
		    	na_field[1] = 0.5*(1.0 + xa[i]);
		...
		case 3:
		{
			for (int i = 0; i < fNumIP; i++)	
			{
				...
				/* Na */
				na_field[0] = -xa[i]*0.5*(1.0 - xa[i]);
				na_field[1] =  xa[i]*0.5*(1.0 + xa[i]);
				na_field[2] =  (1.0 - xa[i])*(1.0 + xa[i]);
		
				/* Na,x */
				nax_field[0] = -0.5 + xa[i];
				nax_field[1] =  0.5 + xa[i];
				nax_field[2] = -2.0*xa[i];
...
void C0_LineT::Jacobian(const LocalArrayT& nodal, const dArray2DT& LDNaX,
							   dMatrixT& jac)
{
	// jacobian
	jac = 0.;
