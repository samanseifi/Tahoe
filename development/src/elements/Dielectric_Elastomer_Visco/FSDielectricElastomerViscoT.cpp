...
/* Destructor */
  FSDielectricElastomerViscoT::~FSDielectricElastomerViscoT()
  {
	  if (0 != fFSDEMatSupportVisco) delete fFSDEMatSupportVisco;
  }

  //
  // specify parameters needed by the interface
  //
  void FSDielectricElastomerViscoT::DefineParameters(ParameterListT& list) const
  {
//  	cout << "FSDielectricElastomerViscoT::DefineParameters" << endl;
    // inherited
  ...
  //
  void FSDielectricElastomerViscoT::TakeParameterList(const ParameterListT& list)
  {
    //
    // inherited
    //
    ...
    if (!fElectricScalarPotentialField) {
      ExceptionT::GeneralFail("FSDielectricElastomerViscoT::TakeParameterList",
  ...
  //
  MaterialSupportT*
  FSDielectricElastomerViscoT::NewMaterialSupport(MaterialSupportT* p) const
  {
    //
    // allocate
    //
    if (!p) p = new FSDEMatSupportViscoT(1, NumIP());
    ...
    //
    FSDEMatSupportViscoT* ps = dynamic_cast<FSDEMatSupportViscoT*> (p);
  ...
  //
  MaterialListT*
  FSDielectricElastomerViscoT::NewMaterialList(const StringT& name, int size)
  {
    ...
    if (size > 0) {
      ...
      if (nsd == 2)
      	mlp = new FSSolidMatList2DT(size, *fFSDEMatSupportVisco);
	  else if (nsd == 3)
		mlp = new FSSolidMatList3DT(size, *fFSDEMatSupportVisco); 
    }
	else {
		if (nsd == 2)
  ...
  void FSDielectricElastomerViscoT::SetGlobalShape()
  {
    //
    // inherited
    //
  ...
  //
  void FSDielectricElastomerViscoT::CurrElementInfo(ostream& out) const
  {
    //
    // inherited
    //
...
// 
  bool FSDielectricElastomerViscoT::NextElement()
  {
    ...
    if (isThereNext == true) {
      ...
      fCurrMaterial = dynamic_cast<FSDEMatViscoT*> (pMaterial);
    }
  ...
  //
  void FSDielectricElastomerViscoT::SetLocalArrays()
  {
    //
    ...
    if (0 == esp) {
      ...
      std::cout << std::endl;
      std::cout << "FSDielectricElastomerViscoT::SetLocalArrays: ";
  ...
  //
  //
  void FSDielectricElastomerViscoT::Equations(AutoArrayT<const iArray2DT*>& eq_1,
...
void FSDielectricElastomerViscoT::AddNodalForce(const FieldT& field, int node, dArrayT& force)
{
	/* not my field */
//	if (&field != &(Field())) return;
  ...
  //
  void FSDielectricElastomerViscoT::Set_B_C(const dArray2DT& DNaX, dMatrixT& B_C)
  {
  ...
  // Geometric stiffness
  //
  void FSDielectricElastomerViscoT::AccumulateGeometricStiffness(dMatrixT& Kg,
...
/* calculate the LHS of residual, or element stiffness matrix */
  void FSDielectricElastomerViscoT::FormStiffness(double constK)
  {
...
void FSDielectricElastomerViscoT::FormMass(MassTypeT mass_type, double constM, bool axisymmetric, const double* ip_weight)
{
...
void FSDielectricElastomerViscoT::FormMa(MassTypeT mass_type, double constM, bool axisymmetric, 
	const LocalArrayT* nodal_values, const dArray2DT* ip_values, const double* ip_weight)
{
	const char caller[] = "FSDielectricElastomerViscoT::FormMa";
...
void FSDielectricElastomerViscoT::MassMatrix()
{
...
/* Compute RHS, or residual of element equations */
  void FSDielectricElastomerViscoT::FormKd(double constK)
  {  	
	/* element preliminaries */
    const int nsd = NumSD();
    const int nen = NumElementNodes();
...
//	  SIJ += SIJ_NEQ;
  ...
  // values
  //
  void FSDielectricElastomerViscoT::ComputeOutput(const iArrayT& n_codes,
