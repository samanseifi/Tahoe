...
namespace Tahoe {
  ...
  FSDielectricElastomerQ1P0T::FSDielectricElastomerQ1P0T(
      const ElementSupportT& support) :
    FiniteStrainT(support), fFSDEMatSupportQ1P0(0), fCurrMaterial(0),
    fLocScalarPotential(LocalArrayT::kESP), fElectricScalarPotentialField(0),
    fLocCurrCoords(LocalArrayT::kCurrCoords)
  {
    SetName("dielectric_elastomer_Q1P0");
  }
...
/* Destructor */
  FSDielectricElastomerQ1P0T::~FSDielectricElastomerQ1P0T()
  {
  	  delete fCurrShapes;
  	  fCurrShapes = NULL;
	  if (0 != fFSDEMatSupportQ1P0) delete fFSDEMatSupportQ1P0;
  }

  // specify parameters needed by the interface  
  void FSDielectricElastomerQ1P0T::DefineParameters(ParameterListT& list) const
  {
    // inherited
...
//	list.RemoveParameter("store_shapefunctions");   
  ...
  void FSDielectricElastomerQ1P0T::TakeParameterList(const ParameterListT& list)
  {
    // inherited
    ...
    if (!fElectricScalarPotentialField) {
      ExceptionT::GeneralFail("FSDielectricElastomerQ1P0T::TakeParameterList",
	...
	/* Q1P0 STUFF */
	const char caller[] = "FSDielectricElastomerQ1P0T::TakeParameterList";	
...
GlobalT::SystemTypeT FSDielectricElastomerQ1P0T::TangentType(void) const
{
	/* Define LHS type based upon analysis type, i.e. static vs. dynamic */
 	int order = fIntegrator->Order();
 	if (order == 2)
...
void FSDielectricElastomerQ1P0T::CloseStep(void)
{
	/* inherited */
...
GlobalT::RelaxCodeT FSDielectricElastomerQ1P0T::ResetStep(void)
{
	/* inherited */
...
void FSDielectricElastomerQ1P0T::ReadRestart(istream& in)
{
	/* inherited */
...
void FSDielectricElastomerQ1P0T::WriteRestart(ostream& out) const
{
	/* inherited */
  ...
  MaterialSupportT*
  FSDielectricElastomerQ1P0T::NewMaterialSupport(MaterialSupportT* p) const
  {
    // allocate
    if (!p) p = new FSDEMatSupportQ1P0T(1, NumIP());
    
    // inherited initializations
    FiniteStrainT::NewMaterialSupport(p);

    // set parent class fields
    FSDEMatSupportQ1P0T* ps = dynamic_cast<FSDEMatSupportQ1P0T*> (p);
  ...
  MaterialListT*
  FSDielectricElastomerQ1P0T::NewMaterialList(const StringT& name, int size)
  {
    ...
    if (size > 0) {
   	  	...
   	  	/* material support */
      	if (0 == fFSDEMatSupportQ1P0) {
        	fFSDEMatSupportQ1P0 = dynamic_cast<FSDEMatSupportQ1P0T*> (NewMaterialSupport());

        if (0 == fFSDEMatSupportQ1P0) {
          ExceptionT::GeneralFail("FSDielectricElastomerQ1P0T::NewMaterialList");
        }
      }
      if (nsd == 2)
      	mlp = new FSSolidMatList2DT(size, *fFSDEMatSupportQ1P0);
	  else if (nsd == 3)
		mlp = new FSSolidMatList3DT(size, *fFSDEMatSupportQ1P0); 
    }
	else {
		if (nsd == 2)
...
void FSDielectricElastomerQ1P0T::SetShape(void)
{
	/* inherited */
  ...
  void FSDielectricElastomerQ1P0T::SetGlobalShape()
  {
    // inherited
    FiniteStrainT::SetGlobalShape();
  ...
  void FSDielectricElastomerQ1P0T::CurrElementInfo(ostream& out) const
  {
    // inherited
...
//  increment current element - for ComputeOutput
  bool FSDielectricElastomerQ1P0T::NextElement()
  {
    ...
    if (isThereNext == true) {
      ...
      fCurrMaterial = dynamic_cast<FSDEMatQ1P0T*> (pMaterial);
    }
  ...
  void FSDielectricElastomerQ1P0T::SetLocalArrays()
  {
    ...
    if (0 == esp) {
      ...
      std::cout << std::endl;
      std::cout << "FSDielectricElastomerQ1P0T::SetLocalArrays: ";
  ...
  //
  void FSDielectricElastomerQ1P0T::Equations(AutoArrayT<const iArray2DT*>& eq_1,
  ...
  //
  int FSDielectricElastomerQ1P0T::TotalNumDOF() const
  {
 	int mechdof = 3;
 	int elecdof = 1;
  ...
  const dArrayT&
  FSDielectricElastomerQ1P0T::ElectricField() const
  {
  ...
  //
  const dArrayT&
  FSDielectricElastomerQ1P0T::ElectricField(int ip) const
  {
    return fE_List[ip];
  }

/* accumulate the residual force on the specified node */
void FSDielectricElastomerQ1P0T::AddNodalForce(const FieldT& field, int node, dArrayT& force)
{
	/* not my field */
...
/* calculate the LHS of residual, or element stiffness matrix */
  void FSDielectricElastomerQ1P0T::FormStiffness(double constK)
  {
...
/* Compute RHS, or residual of element equations */
  void FSDielectricElastomerQ1P0T::FormKd(double constK)
  {  	
...
void FSDielectricElastomerQ1P0T::FormMass(MassTypeT mass_type, double constM, bool axisymmetric, const double* ip_weight)
{
...
void FSDielectricElastomerQ1P0T::FormMa(MassTypeT mass_type, double constM, bool axisymmetric, 
	const LocalArrayT* nodal_values, const dArray2DT* ip_values, const double* ip_weight)
{
	const char caller[] = "FSDielectricElastomerQ1P0T::FormMa";
...
void FSDielectricElastomerQ1P0T::MassMatrix()
{
...
void FSDielectricElastomerQ1P0T::SetMeanGradient(dArray2DT& mean_gradient, double& H, double& v) const
{
...
void FSDielectricElastomerQ1P0T::bSp_bRq_to_KSqRp(const dMatrixT& b, dMatrixT& K) const
{
  ...
  void FSDielectricElastomerQ1P0T::ComputeOutput(const iArrayT& n_codes,
