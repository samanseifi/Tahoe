...
namespace Tahoe {
  ...
  FSDielectricElastomerQ1P0ViscoT::FSDielectricElastomerQ1P0ViscoT(
      const ElementSupportT& support) :
    FiniteStrainT(support), fFSDEMatSupportQ1P0Visco(0), fCurrMaterial(0),
    fLocScalarPotential(LocalArrayT::kESP), fElectricScalarPotentialField(0),
    fLocCurrCoords(LocalArrayT::kCurrCoords)
  {
    SetName("dielectric_elastomer_Q1P0_visco");
  }
...
/* Destructor */
  FSDielectricElastomerQ1P0ViscoT::~FSDielectricElastomerQ1P0ViscoT()
  {
  	  delete fCurrShapes;
  	  fCurrShapes = NULL;
	  if (0 != fFSDEMatSupportQ1P0Visco) delete fFSDEMatSupportQ1P0Visco;
  }

  // specify parameters needed by the interface  
  void FSDielectricElastomerQ1P0ViscoT::DefineParameters(ParameterListT& list) const
  {
    // inherited
...
//	list.RemoveParameter("store_shapefunctions");   
  ...
  void FSDielectricElastomerQ1P0ViscoT::TakeParameterList(const ParameterListT& list)
  {
    // inherited
    ...
    if (!fElectricScalarPotentialField) {
      ExceptionT::GeneralFail("FSDielectricElastomerQ1P0ViscoT::TakeParameterList",
	...
	fCauchyStressneq.Dimension(nsd);
	...
	/* Q1P0 STUFF */
	const char caller[] = "FSDielectricElastomerQ1P0ViscoT::TakeParameterList";	
...
GlobalT::SystemTypeT FSDielectricElastomerQ1P0ViscoT::TangentType(void) const
{
	/* Define LHS type based upon analysis type, i.e. static vs. dynamic */
 	int order = fIntegrator->Order();
 	if (order == 2)
...
void FSDielectricElastomerQ1P0ViscoT::CloseStep(void)
{
	/* inherited */
...
GlobalT::RelaxCodeT FSDielectricElastomerQ1P0ViscoT::ResetStep(void)
{
	/* inherited */
...
void FSDielectricElastomerQ1P0ViscoT::ReadRestart(istream& in)
{
	/* inherited */
...
void FSDielectricElastomerQ1P0ViscoT::WriteRestart(ostream& out) const
{
	/* inherited */
  ...
  MaterialSupportT*
  FSDielectricElastomerQ1P0ViscoT::NewMaterialSupport(MaterialSupportT* p) const
  {
    // allocate
    if (!p) p = new FSDEMatSupportQ1P0ViscoT(1, NumIP());
    
    // inherited initializations
    FiniteStrainT::NewMaterialSupport(p);

    // set parent class fields
    FSDEMatSupportQ1P0ViscoT* ps = dynamic_cast<FSDEMatSupportQ1P0ViscoT*> (p);
  ...
  MaterialListT*
  FSDielectricElastomerQ1P0ViscoT::NewMaterialList(const StringT& name, int size)
  {
    ...
    if (size > 0) {
   	  	...
   	  	/* material support */
      	if (0 == fFSDEMatSupportQ1P0Visco) {
        	fFSDEMatSupportQ1P0Visco = dynamic_cast<FSDEMatSupportQ1P0ViscoT*> (NewMaterialSupport());

        if (0 == fFSDEMatSupportQ1P0Visco) {
          ExceptionT::GeneralFail("FSDielectricElastomerQ1P0ViscoT::NewMaterialList");
        }
      }
      if (nsd == 2)
      	mlp = new FSSolidMatList2DT(size, *fFSDEMatSupportQ1P0Visco);
	  else if (nsd == 3)
		mlp = new FSSolidMatList3DT(size, *fFSDEMatSupportQ1P0Visco); 
    }
	else {
		if (nsd == 2)
...
void FSDielectricElastomerQ1P0ViscoT::SetShape(void)
{
	/* inherited */
  ...
  void FSDielectricElastomerQ1P0ViscoT::SetGlobalShape()
  {
    // inherited
    FiniteStrainT::SetGlobalShape();
  ...
  void FSDielectricElastomerQ1P0ViscoT::CurrElementInfo(ostream& out) const
  {
    // inherited
...
//  increment current element - for ComputeOutput
  bool FSDielectricElastomerQ1P0ViscoT::NextElement()
  {
    ...
    if (isThereNext == true) {
      ...
      fCurrMaterial = dynamic_cast<FSDEMatQ1P0ViscoT*> (pMaterial);
    }
  ...
  void FSDielectricElastomerQ1P0ViscoT::SetLocalArrays()
  {
    ...
    if (0 == esp) {
      ...
      std::cout << std::endl;
      std::cout << "FSDielectricElastomerQ1P0ViscoT::SetLocalArrays: ";
  ...
  //
  void FSDielectricElastomerQ1P0ViscoT::Equations(AutoArrayT<const iArray2DT*>& eq_1,
  ...
  //
  int FSDielectricElastomerQ1P0ViscoT::TotalNumDOF() const
  {
 	int mechdof = 3;
 	int elecdof = 1;
  ...
  const dArrayT&
  FSDielectricElastomerQ1P0ViscoT::ElectricField() const
  {
  ...
  //
  const dArrayT&
  FSDielectricElastomerQ1P0ViscoT::ElectricField(int ip) const
  {
    return fE_List[ip];
  }

/* accumulate the residual force on the specified node */
void FSDielectricElastomerQ1P0ViscoT::AddNodalForce(const FieldT& field, int node, dArrayT& force)
{
	/* not my field */
...
/* calculate the LHS of residual, or element stiffness matrix */
  void FSDielectricElastomerQ1P0ViscoT::FormStiffness(double constK)
  {
...
/* Compute RHS, or residual of element equations */
  void FSDielectricElastomerQ1P0ViscoT::FormKd(double constK)
  {  	
...
void FSDielectricElastomerQ1P0ViscoT::FormMass(MassTypeT mass_type, double constM, bool axisymmetric, const double* ip_weight)
{
...
void FSDielectricElastomerQ1P0ViscoT::FormMa(MassTypeT mass_type, double constM, bool axisymmetric, 
	const LocalArrayT* nodal_values, const dArray2DT* ip_values, const double* ip_weight)
{
	const char caller[] = "FSDielectricElastomerQ1P0ViscoT::FormMa";
...
void FSDielectricElastomerQ1P0ViscoT::MassMatrix()
{
...
void FSDielectricElastomerQ1P0ViscoT::SetMeanGradient(dArray2DT& mean_gradient, double& H, double& v) const
{
...
void FSDielectricElastomerQ1P0ViscoT::bSp_bRq_to_KSqRp(const dMatrixT& b, dMatrixT& K) const
{
  ...
  void FSDielectricElastomerQ1P0ViscoT::ComputeOutput(const iArrayT& n_codes,
...
//	dArrayT ndElectricScalarPotential;
    ...
    if (e_codes[IP_ELEC_FLD]) {
      ipElectricField.Alias(NumIP(), NumSD() + 1, pall);
      pall += (NumIP() + 1) * NumSD();
    }
