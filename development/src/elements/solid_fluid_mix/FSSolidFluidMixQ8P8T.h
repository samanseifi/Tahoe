...
inline const ShapeFunctionT& FSSolidFluidMixQ8P8T::ShapeFunctionDispl(void) const
{
	...
	if (!fShapes_displ)
	    ExceptionT::GeneralFail("FSSolidFluidMixQ8P8T::ShapeFunctionDispl", "no displ shape functions");
#endif
...
#if __option(extended_errorcheck)
	if (!fShapes_press)
	    ExceptionT::GeneralFail("FSSolidFluidMixQ8P8T::ShapeFunctionPress", "no press shape functions");
#endif
	return *fShapes_press;
}

/* return the geometry code */
inline GeometryT::CodeT FSSolidFluidMixQ8P8T::GeometryCode(void) const
