/* Id: FSSolidFluidMixQ8P8T.cpp,v 1.6 2008/24/07 19:55:23 ebrahimi Exp $ */
...
/* constructor */
FSSolidFluidMixQ8P8T::FSSolidFluidMixQ8P8T(const ElementSupportT& support):
...
//88888888 preconditioner
    fKdd_new(ElementMatrixT::kNonSymmetric),
    fKdtheta_new(ElementMatrixT::kNonSymmetric),
...
/* destructor */
FSSolidFluidMixQ8P8T::~FSSolidFluidMixQ8P8T(void)
{
...
void FSSolidFluidMixQ8P8T::Echo_Input_Data(void) {
    ...
    cout << "#######################################################" << endl;
    cout << "############### ECHO FSSolidFluidMixQ8P8 DATA #########################" << endl;
    cout << "#######################################################" << endl;
...
void FSSolidFluidMixQ8P8T::Equations(AutoArrayT<const iArray2DT*>& eq_d,
...
void FSSolidFluidMixQ8P8T::LHSDriver(GlobalT::SystemTypeT)
{
...
void FSSolidFluidMixQ8P8T::Select_Equations (const int &iBalLinChoice, const int &iMassBalChoice )
{
    ...
    {
    default :
	cout << "FSSolidFluidMixQ8P8T::Select_Equations() .. currently only one linear momentum balance for mixture \n";
	break;
    }
    ...
    {
    default :
	cout << "FSSolidFluidMixQ8P8T::Select_Equations() .. currently only one mass balance equation for mixture \n";
	break;
    }
 ...
 * given group. */
bool FSSolidFluidMixQ8P8T::InGroup(int group) const
{
...
{
	const char caller[] = "FSSolidFluidMixQ8P8T::ResetStep";
...
/*
GlobalT::RelaxCodeT FSSolidFluidMixQ8P8T::RelaxSystem(void)
{
	const char caller[] = "FSSolidFluidMixQ8P8T::RelaxSystem";
...
/*
void FSSolidFluidMixQ8P8T::SetStatus(const ArrayT<ElementCardT::StatusT>& status)
{
...
void FSSolidFluidMixQ8P8T::InitialCondition(void)
{
	/* inherited */
...
void FSSolidFluidMixQ8P8T::AddNodalForce(const FieldT& field, int node, dArrayT& force)
{
    const char caller[] = "FSSolidFluidMixQ8P8T::AddNodalForce";
...
double FSSolidFluidMixQ8P8T::InternalEnergy ( void )
{
//not implemented
    return 0.0;
}
...
void FSSolidFluidMixQ8P8T::WriteRestart(ostream& out) const
{
    /* inherited */
...
void FSSolidFluidMixQ8P8T::ReadRestart(istream& in)
{
    /* inherited */
...
void FSSolidFluidMixQ8P8T::RegisterOutput(void)
{
    /* collect block ID's */
...
void FSSolidFluidMixQ8P8T::WriteOutput(void)
{
    bStep_Complete=1;
    RHSDriver();
    bStep_Complete=0;
...
//---------------------------------------------------------------------
/* form group contribution to the stiffness matrix and RHS */
void FSSolidFluidMixQ8P8T::RHSDriver_monolithic(void)
{
    const char caller[] = "FSSolidFluidMixQ8P8T::RHSDriver_monolithic";
    if (fDispl->Group() != fPress->Group())
	...
	*/
    ...
    while (NextElement())
    {
		...
		fFtheta_int_Stab_vector_2 = 0.0;		// change, \delta J term
		fFtheta_int_Stab_vector_BP = 0.0;		// change, B-P method
		...
		double fElement_volume = 0.0;	// change, current volume
		...
		*/
		...
		{ //-- Still Iterating
			...
			while (fShapes_displ->NextIP() && fShapes_press->NextIP())
			{
				...
				fTemp_vector_nen_press.SetToScaled(scale_const,fShapeFluid);		// change scale_vol
				/* accumulate */
				...
				double fPore_fluid_pressure_projected_temp = scale_const*fP_f;      // change origin: scale_vol
				/* accumulate */
				fPore_fluid_pressure_projected += fPore_fluid_pressure_projected_temp;

				/* [fElement_volume] will be formed */
				/* accumulate */
				fElement_volume += scale_const*J;		// change, current volume
				...
				*/
				...
				scale = J*(fC3 + fRho_0)*integrate_param*scale_const;		// change, origin: J*(fC3 + fRho_0)
				fTemp_matrix_ndof_se_x_ndof_se *= scale;
				/* accumulate */
				...
				// stabilization terms calculation, based on Brezzi-Pitkaranta method

				/* [fShapeFluidgrad] will be formed */
				fShapeFluidgrad.MultATB(fDeformation_Gradient_Inverse,fShapeFluidGrad);

				/* [fK_thetatheta_HStab_matrix_BP] will be formed */
				fTemp_matrix_nen_press_x_nen_press.MultATB(fShapeFluidgrad,fShapeFluidgrad);	// change origin: fShapeFluidgrad, fShapeFluidgrad
				scale = scale_const*gamma_delta_t*fMaterial_Params[kAlpha]*J;			// change origin: *J
				fTemp_matrix_nen_press_x_nen_press *= scale;
				/* accumulate */
			...
			while (fShapes_displ->NextIP() && fShapes_press->NextIP())
			{
				...
				dArrayT SolidIPCoordinate(n_sd);
				fShapes_displ->IPCoords(SolidIPCoordinate);
				fShapes_press->IPCoords(FluidIPCoordinate);
		    ...
		    if (time==0 && kInitialConditionType==1 && kAnalysisType!=0)
			{
		    	...
		    	fFtheta_int_C1_vector = 0.0;
		    	fFtheta_int_C2_vector = 0.0;
		    	fFtheta_int_Stab_vector = 0.0;			// change
		    	fFtheta_int_Stab_vector_2 = 0.0;		// change \delta J term
		    	fFtheta_int_Stab_vector_BP = 0.0;		// change B-P method
			}
			else if (time>=0 && kAnalysisType==0)
...
void FSSolidFluidMixQ8P8T::DefineParameters(ParameterListT& list) const
{
    ...
    double alpha_k, H_c, Phi, Psi, R, Beta, kappa0, c0, Alpha;
...
{
    const char caller[] = "FSSolidFluidMixQ8P8T::TakeParameterList";
	...
	*/
	...
	if (n_sd == 3 && fGeometryCode_displ == GeometryT::kHexahedron)
    {
...
//888888888888888 preconditioner
fKdd_new.Dimension 			( n_en_displ_x_n_sd, n_en_displ_x_n_sd );
fKdtheta_new.Dimension 		( n_en_displ_x_n_sd, n_en_press );
fKthetad_new.Dimension 		( n_en_press, n_en_displ_x_n_sd );
fKthetatheta_new.Dimension 	( n_en_press, n_en_press );
...
fFtheta_int_new.Dimension 	( n_en_press );
    ...
    fShapeFluidgrad.Dimension (n_sd, n_en_press);	// change B-P method
    ...
    fFtheta_int_Stab_vector_2.Dimension(n_en_press);					// change \delta J term
    fFtheta_int_Stab_vector_BP.Dimension(n_en_press);					// change B-P method
...
// output to debug
    ...
    fileK_thetatheta_HStab_matrix.open("Q8P8_fileK_thetatheta_HStab_matrix_0_alpha");

    fileShape_fluid_projected.open("Q8P8_fileShape_fluid_projected_0_alpha");
    filePore_fluid_pressure_projected.open("Q8P8_filePore_fluid_pressure_projected_0_alpha");

    fileFtheta_int.open("Q8P8_fileFtheta_int_0_alpha");
    fileKthetatheta.open("Q8P8_fileKthetatheta_0_alpha");
*/
...
void FSSolidFluidMixQ8P8T::DefineSubs(SubListT& sub_list) const
{
    ...
    /* element blocks */
    sub_list.AddSub("total_lagrangian_solid_fluid_mix_q8p8_element_block");
...
ParameterInterfaceT* FSSolidFluidMixQ8P8T::NewSub(const StringT& name) const
{
    ...
    FSSolidFluidMixQ8P8T* non_const_this = const_cast<FSSolidFluidMixQ8P8T*>(this);
    ...
    else if (name == "total_lagrangian_solid_fluid_mix_q8p8_natural_bc") /* traction bc */
    {
    ...
    else if (name == "total_lagrangian_solid_fluid_mix_q8p8_element_block")
    {
...
void FSSolidFluidMixQ8P8T::SetTractionBC(void)
{
...
void FSSolidFluidMixQ8P8T::ApplyTractionBC(void)
{
...
void FSSolidFluidMixQ8P8T::	Form_fluid_shape_functions(const double* &shapes_press_X)
{
    fShapeFluid = 0.0;
    for (int i=0; i<8; i++)
...
void FSSolidFluidMixQ8P8T::	Form_deformation_gradient_tensor(void)
{
...
void FSSolidFluidMixQ8P8T::Form_Grad_grad_transformation_matrix(void)
{
...
void FSSolidFluidMixQ8P8T::Form_fDefGradT_9x9_matrix(void)
{
...
void FSSolidFluidMixQ8P8T::Form_effective_kirchhoff_stress_vector()
{
...
void FSSolidFluidMixQ8P8T::Form_Im_temp_matrix()
{
    fIm_temp_matrix = 0.0;
...
void FSSolidFluidMixQ8P8T::Form_Hbar_temp_matrix()
{
...
void FSSolidFluidMixQ8P8T::Form_Ell_temp_matrix()
{
...
void FSSolidFluidMixQ8P8T::Form_HbarTau_temp_matrix()
{
...
// for LINEAR elasticity
void FSSolidFluidMixQ8P8T::Form_EllTau_temp_matrix()
{
...
// for LINEAR elasticity
void FSSolidFluidMixQ8P8T::Form_BotimesB_temp_matrix()
{
...
// for LINEAR elasticity
void FSSolidFluidMixQ8P8T::Form_BodotB_temp_matrix()
{
...
void FSSolidFluidMixQ8P8T::Form_Jmath_temp_matrix(void)
{
...
void FSSolidFluidMixQ8P8T::Form_Jmath_prim_temp_matrix(void)
{
...
void FSSolidFluidMixQ8P8T::Form_Wp_temp_matrix(void)
{
    for (int i=0; i<3; i++)
    {
	for (int j=0; j<9; j++)
...
void FSSolidFluidMixQ8P8T::Form_Wp_prim_temp_matrix(void)
{
    for (int i=0; i<3; i++)
    {
	for (int j=0; j<9; j++)
...
void FSSolidFluidMixQ8P8T::Form_C_matrix(const double& J_Prim)
{
    double C_IJKL;
    int row,col;
    for (int I=0; I<3; I++)
...
void FSSolidFluidMixQ8P8T::Form_c_matrix()
{
    double c_ijkl;
    int row,col;
    for (int i=0; i<3; i++)
...
void FSSolidFluidMixQ8P8T::Put_values_In_dArrayT_vector(const dArray2DT &f2DArrayT,const int& e,const int& IP,dArrayT& fArrayT)
{
...
void FSSolidFluidMixQ8P8T::Form_Varsigma_temp_matrix(void)
{
...
void FSSolidFluidMixQ8P8T::Form_I_ijkl_matrix(void)
{
...
void FSSolidFluidMixQ8P8T::Form_Aleph_temp_matrix(const int& IP)
{
...
void FSSolidFluidMixQ8P8T::Form_Imath_temp_matrix(void)
{
