...
namespace Tahoe {
  ...
  FSDEMatQ1P0ViscoT::FSDEMatQ1P0ViscoT():
    ParameterInterfaceT("Finite Strain Dielectric Elastomer Viscoelastic Q1P0"),
	fFSDEMatSupportQ1P0Visco(0), 
    fSpectralDecompRef(3),
    fSpectralDecompSpat(3)
  {
    SetName(FSDEMatQ1P0ViscoT::Name);
    Initialize();	// MAY CALL LATER IN TAKEPARAMETERLIST?
	/*set default value*/ 
	/*overide in derived element classes before calling *
	 *RGViscoelasticity::TakeParameterList               */  
  }

  //
  static const char DE[] = "Dielectric_Elastomer_Viscoelastic_Q1P0";
  const char* FSDEMatQ1P0ViscoT::Name = DE;
  const int kNSD       = 3;
  const int kNumDOF    = 3;
  ...
  //
  void FSDEMatQ1P0ViscoT::Initialize()
  {
    fMu = 0.0;
  ...
  //
  void FSDEMatQ1P0ViscoT::DefineParameters(ParameterListT& list) const
  {
  ...
  //
  void FSDEMatQ1P0ViscoT::TakeParameterList(const ParameterListT& list)
  {
  	/* inherited */
	...
	StringT caller = "FSDEMatQ1P0ViscoT::TakeParameterList";
	...
	if (num_neq != num_shear_visc || num_neq != num_bulk_visc)
		ExceptionT::GeneralFail("FSDEMatQ1P0ViscoT::TakeParameterList", 
  ...
  void FSDEMatQ1P0ViscoT::DefineSubs(SubListT& sub_list) const
  {
	/* inherited */
  ...
  void FSDEMatQ1P0ViscoT::SetElectricPermittivity(double epsilon)
  {	
    fElectricPermittivity = epsilon;
  }

  // Get electrical permittivity
  double FSDEMatQ1P0ViscoT::GetElectricPermittivity() const
  {
    return fElectricPermittivity;
  }

  //
  void FSDEMatQ1P0ViscoT::SetFSDEMatSupportQ1P0Visco(
      const FSDEMatSupportQ1P0ViscoT* support)
  {
    fFSDEMatSupportQ1P0Visco = support;
  }

  //
  const dArrayT FSDEMatQ1P0ViscoT::ElectricField()
  {
    fElectricField = fFSDEMatSupportQ1P0Visco->ElectricField();
    return fElectricField;
  }

  //
  const dArrayT FSDEMatQ1P0ViscoT::ElectricField(int ip)
  {
    fElectricField = fFSDEMatSupportQ1P0Visco->ElectricField(ip);
    return fElectricField;
  }

  //
  const dMatrixT FSDEMatQ1P0ViscoT::RightCauchyGreenDeformation()
  {
  ...
  double FSDEMatQ1P0ViscoT::StrainEnergyDensity()
  {
	/* calculates equilibrium part of electromechanical energy density */
	const dMatrixT& F = MechanicalDeformation();
	const dMatrixT& Fm = F_mechanical();
...
if (fNumProcess > 0)
{
	...
	for (int i = 0; i < fNumProcess; i++)
	{
 	...
 	energy1 = me_energy_q1p0visco(fParams.Pointer(), E.Pointer(), C.Pointer(), Fm.Pointer(), J, I1); 
 	energy1+=energy;
	return(energy1);
  ...
  const dMatrixT& FSDEMatQ1P0ViscoT::C_IJKL()
  {
    const dMatrixT& C = RightCauchyGreenDeformation();
    const dArrayT& E = ElectricField();
    const dMatrixT& F = F_mechanical();
	...
	/* call C function for mechanical part of tangent modulus */
 	mech_tanmod_q1p0visco(fParams.Pointer(), E.Pointer(), C.Pointer(), F.Pointer(), J, I1, fTangentMechanical.Pointer()); 
 	me_tanmod_q1p0visco(fParams.Pointer(), E.Pointer(), C.Pointer(), F.Pointer(), J, fTangentMechanicalElec.Pointer());
 	fTangentMechanical+=fTangentMechanicalElec;
    return fTangentMechanical;
  }

  // Second Piola-Kirchhoff stress (mechanical)
  const dSymMatrixT& FSDEMatQ1P0ViscoT::S_IJ()
  {
    const dMatrixT& C = RightCauchyGreenDeformation();
	const dArrayT& E = ElectricField();
    const dMatrixT& F = F_mechanical();	
	...
	/* call C function for mechanical part of PK2 stress */
 	mech_pk2_q1p0visco(fParams.Pointer(), E.Pointer(), C.Pointer(), F.Pointer(), J, I1, stress_temp.Pointer()); 
	me_pk2_q1p0visco(fParams.Pointer(), E.Pointer(), C.Pointer(), F.Pointer(), J, stress_temp2.Pointer());
  ...
  const dMatrixT& FSDEMatQ1P0ViscoT::E_IJK()
  {
    const dMatrixT& C = RightCauchyGreenDeformation();
	const dArrayT& E = ElectricField();
    const dMatrixT& F = F_mechanical();		
	...
	/* call C function for electromechanical tangent modulus */
 	me_mixedmodulus_q1p0visco(fParams.Pointer(), E.Pointer(),  
  ...
  const dMatrixT& FSDEMatQ1P0ViscoT::e_ijk()
  {
	...
	/* call C function for (spatial) electromechanical tangent modulus */
 	me_mixedmodulus_q1p0viscospatial(fParams.Pointer(), E.Pointer(),  
  ...
  const dMatrixT& FSDEMatQ1P0ViscoT::B_IJ()
  {
    const dMatrixT& C = RightCauchyGreenDeformation();
	double J = C.Det();
	J = sqrt(J);
...
//    const dMatrixT b = B_IJ();
  ...
  const dArrayT& FSDEMatQ1P0ViscoT::D_I()
  {
  	const dMatrixT& C = RightCauchyGreenDeformation();
  	const dArrayT& E = ElectricField();
  	const dMatrixT& F = F_mechanical();
	...
	/* call C function for electric stress (i.e. electric displacement D_{I}) */
 	elec_pk2_q1p0visco(fParams.Pointer(), E.Pointer(),  
  ...
  // Electric field
  const dArrayT& FSDEMatQ1P0ViscoT::E_I()
  {
    return fElectricField;
  }
...
//     fTangentMechanical.SetToScaled(1.0 / J, PushForward(F, CIJKL));
  ...
  double FSDEMatQ1P0ViscoT::Pressure() const
  {
    return 0.0;
  }
...
void  FSDEMatQ1P0ViscoT::PointInitialize(void)
{
...
void FSDEMatQ1P0ViscoT::UpdateHistory(void)
{
	/* current element */
	ElementCardT& element = CurrentElement();	
...
void FSDEMatQ1P0ViscoT::ResetHistory(void)
{
	/* current element */
	ElementCardT& element = CurrentElement();	
	for (int ip = 0; ip < NumIP() && fNumProcess > 0; ip++)
...
GlobalT::SystemTypeT FSDEMatQ1P0ViscoT::TangentType(void) const
{
	/* symmetric by default */
	return GlobalT::kNonSymmetric;
}

const dArrayT& FSDEMatQ1P0ViscoT::Compute_Eigs_v(const int process_id)
{
	fSpectralDecompRef.SpectralDecomp_Jacobi(fC_v[process_id], false);
	return(fSpectralDecompRef.Eigenvalues());
} 

const dArrayT& FSDEMatQ1P0ViscoT::Compute_Eigs_vn(const int process_id)
{
	fSpectralDecompRef.SpectralDecomp_Jacobi(fC_vn[process_id], false);
	return(fSpectralDecompRef.Eigenvalues());
} 

void FSDEMatQ1P0ViscoT::Load(ElementCardT& element, int ip)
{
...
void FSDEMatQ1P0ViscoT::Store(ElementCardT& element, int ip)
{
	/* fetch internal variable array */
	dArrayT& d_array = element.DoubleData();
...
const dMatrixT& FSDEMatQ1P0ViscoT::MechanicalDeformation(void)
{
...
const dMatrixT& FSDEMatQ1P0ViscoT::ThermalDeformation_Inverse(void)
{
...
const dMatrixT& FSDEMatQ1P0ViscoT::c_ijkl_neq()
{
	const dMatrixT& F = MechanicalDeformation();
	if (NumSD() == 2)
	{
...
const dSymMatrixT& FSDEMatQ1P0ViscoT::s_ij_neq()
{
	const dMatrixT& F = MechanicalDeformation();
...
void FSDEMatQ1P0ViscoT::MixedRank4_2D(const dArrayT& a, const dArrayT& b, 
...
void FSDEMatQ1P0ViscoT::MixedRank4_3D(const dArrayT& a, const dArrayT& b, 
	dMatrixT& rank4_ab) const
{
#if __option(extended_errorcheck)
	if (a.Length() != 3 ||
...
void FSDEMatQ1P0ViscoT::SetStateVariables(const int numprocess)
{
...
/* BELOW ARE FUNCTIONS FROM RGSplitT2.cpp */
 ...
 void FSDEMatQ1P0ViscoT::Compute_Calg(const dArrayT& tau_dev, const dSymMatrixT& dtau_dev, const double& tau_m, 
...
void FSDEMatQ1P0ViscoT::ComputeEigs_e(const dArrayT& eigenstretch, dArrayT& eigenstretch_e, 
...
ParameterInterfaceT* FSDEMatQ1P0ViscoT::NewSub(const StringT& name) const
{
	PotentialT* pot = NULL;
	if (name == "neo-hookean")
		pot = new NeoHookean;
	...
	else return(FSDEMatQ1P0ViscoT::NewSub(name));
}
