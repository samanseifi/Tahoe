/* $Id: RGBaseT.cpp,v 1.1 2006/10/30 23:32:05 thao Exp $ */
...
void  RGBaseT::PointInitialize(void)
{
...
void RGBaseT::UpdateHistory(void)
{
	/* current element */
	ElementCardT& element = CurrentElement();	
	for (int ip = 0; ip < NumIP(); ip++)
...
void RGBaseT::ResetHistory(void)
{
	/* current element */
	ElementCardT& element = CurrentElement();	
	for (int ip = 0; ip < NumIP(); ip++)
...
GlobalT::SystemTypeT RGBaseT::TangentType(void) const
{
	/* symmetric by default */
	return GlobalT::kNonSymmetric;
}

void RGBaseT::Load(ElementCardT& element, int ip)
{
...
void RGBaseT::Store(ElementCardT& element, int ip)
{
	/* fetch internal variable array */
	dArrayT& d_array = element.DoubleData();
...
void RGBaseT::MixedRank4_2D(const dArrayT& a, const dArrayT& b, 
...
void RGBaseT::MixedRank4_3D(const dArrayT& a, const dArrayT& b, 
	dMatrixT& rank4_ab) const
{
#if __option(extended_errorcheck)
	if (a.Length() != 3 ||
