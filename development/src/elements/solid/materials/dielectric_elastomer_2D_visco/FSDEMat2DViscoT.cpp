...
namespace Tahoe {
  ...
  FSDEMat2DViscoT::FSDEMat2DViscoT():
    ParameterInterfaceT("Finite Strain Dielectric Elastomer 2D Viscoelastic"),
  ...
  //
  void FSDEMat2DViscoT::DefineParameters(ParameterListT& list) const
  {
  ...
  //
  void FSDEMat2DViscoT::TakeParameterList(const ParameterListT& list)
  {
  	/* inherited */
  ...
  void FSDEMat2DViscoT::DefineSubs(SubListT& sub_list) const
  {
	/* inherited */
  ...
  void FSDEMat2DViscoT::SetElectricPermittivity(double epsilon)
  {	
    fElectricPermittivity = epsilon;
  }

  // Get electrical permittivity
  double FSDEMat2DViscoT::GetElectricPermittivity() const
  {
    return fElectricPermittivity;
  }

  //
  void FSDEMat2DViscoT::SetFSDEMatSupport2DVisco(
  ...
  //
  const dArrayT FSDEMat2DViscoT::ElectricField()
  {
  ...
  //
  const dArrayT FSDEMat2DViscoT::ElectricField(int ip)
  {
  ...
  //
  const dMatrixT FSDEMat2DViscoT::RightCauchyGreenDeformation()
  {
  ...
  double FSDEMat2DViscoT::StrainEnergyDensity()
  {
...
//    return fEnergyDensity;
  ...
  const dMatrixT& FSDEMat2DViscoT::C_IJKL()
  {
    const dMatrixT F = F_mechanical();
    dMatrixT C(2);
    C.MultATB(F, F);  
  ...
  const dSymMatrixT& FSDEMat2DViscoT::S_IJ()
  {
    const dMatrixT F = F_mechanical();
    dMatrixT C(2);
    C.MultATB(F, F);  
  ...
  const dMatrixT& FSDEMat2DViscoT::E_IJK()
  {
    const dMatrixT F = F_mechanical();
    dMatrixT C(2);
    C.MultATB(F, F);  
  ...
  const dMatrixT& FSDEMat2DViscoT::B_IJ()
  {
    const dMatrixT F = F_mechanical();
    dMatrixT C(2);
    C.MultATB(F, F);  
  ...
  const dArrayT& FSDEMat2DViscoT::D_I()
  {
    const dMatrixT F = F_mechanical();
    dMatrixT C(2);
    C.MultATB(F, F);  
  ...
  // Electric field
  const dArrayT& FSDEMat2DViscoT::E_I()
  {
    return fElectricField;
  }
  ...
  double FSDEMat2DViscoT::Pressure() const
  {
    return 0.0;
  }
...
void  FSDEMat2DViscoT::PointInitialize(void)
{
...
void FSDEMat2DViscoT::UpdateHistory(void)
{
	/* current element */
	ElementCardT& element = CurrentElement();	
...
void FSDEMat2DViscoT::ResetHistory(void)
{
	/* current element */
	ElementCardT& element = CurrentElement();	
	for (int ip = 0; ip < NumIP() && fNumProcess > 0; ip++)
...
GlobalT::SystemTypeT FSDEMat2DViscoT::TangentType(void) const
{
	/* symmetric by default */
	return GlobalT::kNonSymmetric;
}

const dArrayT& FSDEMat2DViscoT::Compute_Eigs_v(const int process_id)
{
	fSpectralDecompRef.SpectralDecomp_Jacobi(fC_v[process_id], false);
	return(fSpectralDecompRef.Eigenvalues());
} 

const dArrayT& FSDEMat2DViscoT::Compute_Eigs_vn(const int process_id)
{
	fSpectralDecompRef.SpectralDecomp_Jacobi(fC_vn[process_id], false);
	return(fSpectralDecompRef.Eigenvalues());
} 

void FSDEMat2DViscoT::Load(ElementCardT& element, int ip)
{
...
void FSDEMat2DViscoT::Store(ElementCardT& element, int ip)
{
	/* fetch internal variable array */
	dArrayT& d_array = element.DoubleData();
...
const dMatrixT& FSDEMat2DViscoT::MechanicalDeformation(void)
{
...
const dMatrixT& FSDEMat2DViscoT::ThermalDeformation_Inverse(void)
{
...
const dMatrixT& FSDEMat2DViscoT::c_ijkl_neq()
{
...
const dSymMatrixT& FSDEMat2DViscoT::s_ij_neq()
{
	const dMatrixT& F = MechanicalDeformation();
...
void FSDEMat2DViscoT::MixedRank4_2D(const dArrayT& a, const dArrayT& b, 
...
void FSDEMat2DViscoT::MixedRank4_3D(const dArrayT& a, const dArrayT& b, 
	dMatrixT& rank4_ab) const
{
#if __option(extended_errorcheck)
	if (a.Length() != 3 ||
...
void FSDEMat2DViscoT::SetStateVariables(const int numprocess)
{
...
/* BELOW ARE FUNCTIONS FROM RGSplitT2.cpp */
 void FSDEMat2DViscoT::Compute_Calg(const dArrayT& tau_dev, const dSymMatrixT& dtau_dev, const double& tau_m, 
...
void FSDEMat2DViscoT::ComputeEigs_e(const dArrayT& eigenstretch, dArrayT& eigenstretch_e, 
...
ParameterInterfaceT* FSDEMat2DViscoT::NewSub(const StringT& name) const
{
	PotentialT* pot = NULL;
	if (name == "neo-hookean")
		pot = new NeoHookean;
	...
	else return(FSDEMat2DViscoT::NewSub(name));
}
