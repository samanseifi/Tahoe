...
/* constructor */
SMRSSNLHardT::SMRSSNLHardT(ifstreamT& in, int num_ip, double mu, double lambda):
	SMRPrimitiveT(in),
	fNumIP(num_ip),
	fmu(mu),
	flambda(lambda),
 ...
 */
double& SMRSSNLHardT::Yield_f(const dArrayT& Sig, 
...
dArrayT& SMRSSNLHardT::qbar_f(const dArrayT& Sig, const dArrayT& qn, dArrayT& qbar)
{
...
dMatrixT& SMRSSNLHardT::dQdSig2_f(const dArrayT& qn, dMatrixT& dQdSig2)
{
  int i, j;
  dMatrixT I_mat(6,6);
  I_mat = 0.;
...
dArrayT& SMRSSNLHardT::dfdSig_f(const dArrayT& Sig, const dArrayT& qn, dArrayT& dfdSig)
{
  double Sig_p, temp;
  int i; 
   Sig_p = (Sig[0]+Sig[1]+Sig[3])/3.0;
...
dArrayT& SMRSSNLHardT::dQdSig_f(const dArrayT& Sig, const dArrayT& qn, dArrayT& dQdSig)
{
  double Sig_p, temp;
  int i;
...
dArrayT& SMRSSNLHardT::dfdq_f(const dArrayT& Sig, const dArrayT& qn, dArrayT& dfdq)
{
...
dMatrixT& SMRSSNLHardT::dQdSigdq_f(const dArrayT& Sig, const dArrayT& qn, dMatrixT& dQdSigdq)
{
  double Sig_p;
  Sig_p = (Sig[0]+Sig[1]+Sig[3])/3.0;
  dQdSigdq = 0.;
...
dMatrixT& SMRSSNLHardT::dqbardSig_f(const dArrayT& Sig, const dArrayT& qn, dMatrixT& dqbardSig)
{
...
dMatrixT& SMRSSNLHardT::dqbardq_f(const dArrayT& Sig, const dArrayT& qn, dMatrixT& dqbardq)
{
   double Sig_p, A1, B1, A2, A3, A4, dQdP, B2dQdS, B3dQdS;
...
const dMatrixT& SMRSSNLHardT::ModuliDisc(const ElementCardT& element, 
	int ip)
{
	/* initialize */
...
void SMRSSNLHardT::PrintName(ostream& out) const
{
	/* inherited */
	SMRPrimitiveT::PrintName(out);
 ...
 * yield surface */
int SMRSSNLHardT::PlasticLoading(const dSymMatrixT& trialstrain, ElementCardT& element, int ip)
{
	/* not yet plastic */
	if (!element.IsAllocated()) 
