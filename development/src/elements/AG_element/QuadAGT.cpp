...
/* constructor */
QuadAGT::QuadAGT(int numnodes): 
	GeometryBaseT(numnodes, kNumVertexNodes)
{ 
}
	
 /* destructor */
QuadAGT::~QuadAGT(void) 
{
}	
 
 
 /* evaluate the shape functions. */
 void QuadAGT::EvaluateShapeFunctions(const dArrayT& coords, dArrayT& Na) const
 {
     const char caller[] = "QuadAGT::EvaluateShapeFunctions";
	 ...
	 double Gamma0=Gamma(0.0);
	 double Gamma_r=Gamma(r);
	 double Gamma_s=Gamma(s);
	 
	 //int AGType = fQuadAG.GetParameter("AGType");
	 int AGType = 1;
	 
     if(fNumNodes == 9 && AGType == 0)

     {
       double lr1 = (-1*Gamma0+(Gamma0-1.0)*r+ Gamma_r)/(1-2*Gamma0);
       double lr2 = (1+r-2*Gamma_r)/(1-2*Gamma0);
       double lr3 = (-1*Gamma0-1*Gamma0*r+Gamma_r)/(1-2*Gamma0);

       double ls1 = (-1*Gamma0+(Gamma0-1.0)*s+ Gamma_s)/(1-2*Gamma0);
       double ls2 = (1+s-2*Gamma_s)/(1-2*Gamma0);
       double ls3 = (-1*Gamma0-1*Gamma0*s+Gamma_s)/(1-2*Gamma0);
 ...
 void QuadAGT::EvaluateShapeFunctions(const dArrayT& coords, dArrayT& Na, dArray2DT& DNa) const
 {
	 ...
	 double Gamma_r=Gamma(r);
	 double Gamma_s=Gamma(s);
	 double GammaPrim_r=GammaPrim(r);
	 double GammaPrim_s=GammaPrim(s);
...

     {
       ...
       double ls3 = (-1*Gamma0-1*Gamma0*s+Gamma_s)/(1-2*Gamma0);
         ...
         na[0] = lr1*ls1; nax[0]=(Gamma0-1+GammaPrim_r)/(1-2*Gamma0)*ls1;   nay[0]=lr1*(Gamma0-1+GammaPrim_s)/(1-2*Gamma0);
         na[1] = lr3*ls1; nax[1]=(-1*Gamma0+GammaPrim_r)/(1-2*Gamma0)*ls1;  nay[1]=lr3*(Gamma0-1+GammaPrim_s)/(1-2*Gamma0);
         na[2] = lr3*ls3; nax[2]=(-1*Gamma0+GammaPrim_r)/(1-2*Gamma0)*ls3;  nay[2]=lr3*(-1*Gamma0+GammaPrim_s)/(1-2*Gamma0);
         na[3] = lr1*ls3; nax[3]=(Gamma0-1+GammaPrim_r)/(1-2*Gamma0)*ls3;   nay[3]=lr1*(-1*Gamma0+GammaPrim_s)/(1-2*Gamma0);
         na[4] = lr2*ls1; nax[4]=(1-2*GammaPrim_r)/(1-2*Gamma0)*ls1;        nay[4]=lr2*(Gamma0-1+GammaPrim_s)/(1-2*Gamma0);
         na[5] = lr3*ls2; nax[5]=(-1*Gamma0+GammaPrim_r)/(1-2*Gamma0)*ls2;  nay[5]=lr3*(1-2*GammaPrim_s)/(1-2*Gamma0);
         na[6] = lr2*ls3; nax[6]=(1-2*GammaPrim_r)/(1-2*Gamma0)*ls3;        nay[6]=lr2*(-1*Gamma0+GammaPrim_s)/(1-2*Gamma0);
         na[7] = lr1*ls2; nax[7]=(Gamma0-1+GammaPrim_r)/(1-2*Gamma0)*ls2;   nay[7]=lr1*(1-2*GammaPrim_s)/(1-2*Gamma0);
         na[8] = lr2*ls2; nax[8]=(1-2*GammaPrim_r)/(1-2*Gamma0)*ls2;        nay[8]=lr2*(1-2*GammaPrim_s)/(1-2*Gamma0);
     }
     ...
     
     {
       ...
       double ls3 = 0.5*s*(s+1);
         ...
         na[0] = lr1*ls1; nax[0]=(Gamma0-1+GammaPrim_r)/(1-2*Gamma0)*ls1;   nay[0]=lr1*(s-0.5);
         na[1] = lr3*ls1; nax[1]=(-1*Gamma0+GammaPrim_r)/(1-2*Gamma0)*ls1;  nay[1]=lr3*(s-0.5);
         na[2] = lr3*ls3; nax[2]=(-1*Gamma0+GammaPrim_r)/(1-2*Gamma0)*ls3;  nay[2]=lr3*(s+0.5);
         na[3] = lr1*ls3; nax[3]=(Gamma0-1+GammaPrim_r)/(1-2*Gamma0)*ls3;   nay[3]=lr1*(s+0.5);
         na[4] = lr2*ls1; nax[4]=(1-2*GammaPrim_r)/(1-2*Gamma0)*ls1;        nay[4]=lr2*(s-0.5);
         na[5] = lr3*ls2; nax[5]=(-1*Gamma0+GammaPrim_r)/(1-2*Gamma0)*ls2;  nay[5]=lr3*(-2.0*s);
         na[6] = lr2*ls3; nax[6]=(1-2*GammaPrim_r)/(1-2*Gamma0)*ls3;        nay[6]=lr2*(s+0.5);
         na[7] = lr1*ls2; nax[7]=(Gamma0-1+GammaPrim_r)/(1-2*Gamma0)*ls2;   nay[7]=lr1*(-2.0*s);
         na[8] = lr2*ls2; nax[8]=(1-2*GammaPrim_r)/(1-2*Gamma0)*ls2;        nay[8]=lr2*(-2.0*s);
     }
 }

 /* compute local shape functions and derivatives */
 void QuadAGT::SetLocalShape(dArray2DT& Na, ArrayT<dArray2DT>& Na_x,
     dArrayT& weights) const
 {
     const char caller[] = "QuadAGT::SetLocalShape";
     ...
     for (int i = 0; i < numint; i++)
     {
         ...
         QuadAGT::EvaluateShapeFunctions(coords, na, Na_x[i]);
     }
 }

 /* compute gradients of the "bubble" modes */
 void QuadAGT::BubbleModeGradients(ArrayT<dArray2DT>& Na_x) const
 {
     const char caller[] = "QuadAGT::BubbleModeGradients";
 ...
 void QuadAGT::SetExtrapolation(dMatrixT& extrap) const
 {
     const char caller[] = "QuadAGT::SetExtrapolation";
     ...
     if(numnodes == 9 && (numint !=4 && numint != 9))
         ExceptionT::GeneralFail(caller, "requested number of integration points is not supported by quadratic AG element: %d", numint);
 ...
 void QuadAGT::IPGradientTransform(int ip, dMatrixT& transform) const
 {
     const char caller[] = "QuadAGT::IPGradientTransform";
 ...
 * nodes, mid-edge nodes, mid-face nodes */
 void QuadAGT::NodesOnFacet(int facet, iArrayT& facetnodes) const
 {
 // TEMP: not implemented with midside nodes
     const char caller[] = "QuadAGT::NodesOnFacet";
 ...
 void QuadAGT::NumNodesOnFacets(iArrayT& num_nodes) const
 {
 // TEMP: not implemented with midside nodes
     if (fNumNodes != 4 && fNumNodes != 8 && fNumNodes != 9)
         ExceptionT::GeneralFail("QuadAGT::NumNodesOnFacet", "only implemented for 4, 8, and 9 element nodes: %d", fNumNodes);
 ...
 void QuadAGT::NodesOnEdges(iArray2DT& nodes_on_edges) const {
     nodes_on_edges.Dimension(0,0);
 }

 /* returns the nodes on each facet needed to determine neighbors
 * across facets */
 void QuadAGT::NeighborNodeMap(iArray2DT& facetnodes) const
 {
     int dat4[] = {0,1,1,2,2,3,3,0};
 ...
 void QuadAGT::FacetGeometry(ArrayT<CodeT>& facet_geom, iArrayT& facet_nodes) const
 {
 ...
 bool QuadAGT::PointInDomain(const LocalArrayT& coords, const dArrayT& point) const
 {
 ...
 int QuadAGT::IPDomain(int nip, const dArrayT& coords) const
 {
     const char caller[] = "QuadAGT::IPDomain";
 ...
 /* subdomain geometry */
 GeometryT::CodeT QuadAGT::NodalSubDomainGeometry(void) const
 {
     /* limited support */
     if (fNumNodes != 4)
         ExceptionT::GeneralFail("QuadAGT::NodalSubDomainGeometry",
 ...
 int QuadAGT::NodalSubDomainNumPoints(void) const
 {
     /* limited support */
     if (fNumNodes != 4)
         ExceptionT::GeneralFail("QuadAGT::NodalSubDomainGeometry",
 ...
 void QuadAGT::NodalSubDomainCoordinates(const LocalArrayT& coords, int node,
     LocalArrayT& subdomain_coords) const
 {
     const char caller[] = "QuadAGT::NodalSubDomainCoordinates";
     ...
     if (coords.NumberOfNodes() != fNumNodes ||
         ...
         subdomain_coords.NumberOfNodes() != QuadAGT::NodalSubDomainNumPoints())
 ...
 /* sign function */
 int QuadAGT::Sign(double a) const
 {
	if (a<0) return -1;
	if (a>0) return 1;
	else return 0;
 ...
 double QuadAGT::Gamma(double rs) const
 {
	//double m_param = fQuadAG.GetParameter("m");
	//double n_param = fQuadAG.GetParameter("n");
	double m_param = 1;
	double n_param = 1;
    return pow(fabs(1-pow(1-pow((rs+1)/2,2),m_param)),n_param);
 }

 /* calculating Derivative of the AG kernel */
 double QuadAGT::GammaPrim(double rs) const
 {
 	//double m_param = fQuadAG.GetParameter("m");
	//double n_param = fQuadAG.GetParameter("n");
	double m_param = 1;
	double n_param = 1;
    return (Sign(1-pow(1-pow((rs+1)/2,2),m_param))*m_param*n_param*
    pow(fabs(1-pow(1-pow((rs+1)/2,2),m_param)),n_param-1)*pow(1-pow((rs+1)/2,2),m_param-1)*(rs+1)/2);
 }
