...
int main(int argc, char *argv[])
{
  if(argc < 2) {
    cout << endl 
	 ...
	 << "Usage:" << endl
	 << "1) process a single file:  plotpile particle_file" << endl
	 << "   --example: plotpile iso_particle_008" << endl
	 << "2) process multiple files: plotpile particle_file_prefix  first_suffix  last_suffix  suffix_increment" << endl
	 << "   --example: plotpile iso_particle  0  100  5" << endl << endl;
    return -1;
  }	
  ...
  for(n=first; n<=last; n+=incre) {
    ...
    ofs << "ZONE N=" << NODE*TotalNum + eN << ", E=" << ELEM*TotalNum + eE << ", DATAPACKING=POINT, ZONETYPE=FEQUADRILATERAL" << endl;
    for(k = 0; k < TotalNum; ++k) {
      ifs >> id >> type >> a >> b >> c >> x0 >> y0 >> z0 >> l1 >> m1 >> n1 >> l2 >> m2 >> n2 >> l3 >> m3 >> n3
	  >>tmp>>tmp>>tmp>>tmp>>tmp>>tmp>>tmp>>tmp>>tmp>>tmp>>tmp>>tmp;
