...
vtkTahoeGlyph3D* vtkTahoeGlyph3D::New()
{
  ...
  vtkObject* ret = vtkObjectFactory::CreateInstance("vtkTahoeGlyph3D");
  if(ret)
    {
    return (vtkTahoeGlyph3D*)ret;
    }
  // If the factory was unable to create the object, then create it here.
  return new vtkTahoeGlyph3D;
}
...
void vtkTahoeGlyph3D::ExecuteInformation()
{
...
void vtkTahoeGlyph3D::SetNumberOfSources(int num)
{
  // one more because input has index 0.
  this->SetNumberOfInputs(num+1);
}

int vtkTahoeGlyph3D::GetNumberOfSources()
{
...
void vtkTahoeGlyph3D::SetSource(int id, vtkPolyData *pd)
{
  if (id < 0)
    {
...
vtkPolyData *vtkTahoeGlyph3D::GetSource(int id)
{
...
void vtkTahoeGlyph3D::PrintSelf(ostream& os, vtkIndent indent)
{
...
void vtkTahoeGlyph3D::ComputeInputUpdateExtents( vtkDataObject *output )
{
  vtkPolyData *outPd;
