...
H = 4.0
ONSET_THR = 0.08        # amplitude/H threshold above the undamped noise floor (~0.03H)
...
def eps_c_extrap(eps, A):
    """Onset strain.  The undamped runs bifurcate sharply: A sits at the ~0.03H
    noise floor for the whole pre-onset range, then jumps to O(H).  Extrapolation
    over that noise is meaningless, so we take the strain where A first crosses a
    threshold well above the noise floor (ONSET_THR) -- for a sharp jump this is
    essentially the bifurcation strain."""
    on = np.where((A > ONSET_THR) & (eps > 0.05))[0]
...
def analyze(f, gbar):
    ...
    on = np.where((A > ONSET_THR) & (eps > 0.05))[0]       # wrinkle frame (above noise floor)
    ...
    if gbar == 0.0 or not found:
        ...
        lam = 1.0 - eps[k0]                                # in-plane stretch at the wrinkle frame
        lf, la = lf * lam, la * lam                        # reference -> CURRENT (deformed) wavelength
