...
# accurate theory (nlayer_stability, log-spaced wavenumbers)
Ks = np.geomspace(0.03, 6.0, 70)
gb_e = np.linspace(0.0, 5.0, 28)
th_e = np.array([nl.critical([1.0], [H], [], gb * H, Ks, base="sliding")[0] for gb in gb_e])
gb_l = np.linspace(0.1, 5.0, 28)                 # skip gbar=0 (scale-free)
# CURRENT (deformed) wavelength: l_c/H = (1-eps_c) * 2*pi/(K_c H) = lambda_c * 2*pi/(K_c H)
th_l = []
for gb in gb_l:
    r = nl.critical([1.0], [H], [], gb * H, Ks, base="sliding")
    th_l.append(2 * np.pi / r[1] / H * r[2])
th_l = np.array(th_l)
